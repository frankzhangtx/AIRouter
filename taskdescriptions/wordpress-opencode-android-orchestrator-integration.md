...
- 发布状态：`0.5.0` 已于 2026-08-31 公开发布，npm `latest` 当前指向 `0.5.0`；正式接入仍
  固定写出 `@0.5.0`，不使用浮动 tag。
- 验证状态：插件 Node 测试 `134/134`、Shell 事务测试 `42/42` 均已通过；新发现器已在
  真实 WordPress 工程得到预期矩阵。
- 隔离生命周期：`init → doctor → preflight → shadow → upgrade → uninstall` 已在临时 Android
  fixture 中通过；`init` 管理 45 个文件，幂等升级复用 `45/45`，卸载恢复 1 个原文件并移除
  44 个新增文件，无清理警告。

> `0.5.0` 的候选源码提交为 `2160328`，发布证据提交为 `e9cea71`。Registry 回下载包与
> 获批候选包逐字节一致，SHA-256 为
> `6a755b5cb5ae9f3444269aae586a233cf538b5fa8b5d63eab5d7f279b2ed09f1`。正式接入直接使用
> 固定版本 `@0.5.0`；不要使用 `0.4.0`、浮动 `latest` 或来源不明的本地 tarball。
...
| npm Registry | `0.5.0` 已公开发布，`latest` 指向 `0.5.0` |
| 插件安装状态 | WordPress 工程尚未安装插件；当前应执行 `0.5.0` 首次 `init`，不是 `upgrade` |
...
init .
  ...
  ├── 临时 0600 Gradle init script + `gradlew help`
...
不再需要 WordPress 专用配置文件。修改后的插件在真实工程中自动生成：
...
`0.5.0` 已完成发布，WordPress 正式接入不需要再从插件源码构建 tarball。发布时使用的
固定信息如下：

| 项目 | 已验证值 |
| --- | --- |
| 候选源码提交 | `2160328670162beafec617145b2ba6279673e215` |
| 发布证据提交 | `e9cea71030b7c52c56d0e383bc644298bfcac039` |
| 包名与版本 | `@frankzhang2026/opencode-android-orchestrator@0.5.0` |
...
| 包内容 | 134 个文件；190,956 bytes 压缩，925,053 bytes 解包 |
...
```bash
npm view @frankzhang2026/opencode-android-orchestrator@0.5.0 \
  version dist.shasum dist.integrity dist.fileCount dist.unpackedSize
```

以上命令应报告版本 `0.5.0`、npm SHA-1
`5f63d3a63eb613bda20405414422d67c8f6f38b4`、134 个文件和 925,053 bytes 解包大小。
...
`0.5.0` 已正式发布，且 WordPress 当前没有安装 manifest，因此直接执行固定版本的首次初始化：

```bash
cd /Users/zhanglong/files/program/wordpresslocal/WordPress

npx --yes @frankzhang2026/opencode-android-orchestrator@0.5.0 \
  init . \
  --json
```
...
独立审核项，不能悄悄覆盖。`upgrade` 默认保留旧安装的矩阵，不会因为升级到 `0.5.0` 自动刷新；
...
- [ ] 使用固定的 `@frankzhang2026/opencode-android-orchestrator@0.5.0`，不使用浮动 `latest`。
...
- [ ] `doctor`、134 项 Node 测试、42 项 Shell 测试与 shadow-run 均通过。
- [ ] `.automation-plugin/` 已被忽略且未提交。
- [ ] 安装差异经过人工审核后才提交。
- [ ] 提交后工作树干净，严格的 `preflight.sh --source` 通过。
- [ ] `0.4.0`、源码目录、本地 tarball 或来源不明的包没有用于正式接入。
